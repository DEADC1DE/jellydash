...
final class StreamRuleEngineTest extends TestCase
{
    ...
    {
        $condition = [
            ['parameter' => 'user', 'operator' => 'is', 'value' => 'Alpha, user1, Beta', 'type' => 'str'],
        ];
    ...
    public function testLogicStringAndOrGroups(): void
    {
        ...
        $this->assertFalse($this->engine->evaluate($match('user1', 'DirectPlay'), '{1} and {2} and {3}', $this->session));

        // Brackets: {1} and ({2} or {3}) → True and (False or True) = True.
        $this->assertTrue($this->engine->evaluate($match('user1', 'DirectPlay'), '({1} and ({2} or {3}))', $this->session));
    }

    public function testMalformedLogicIsNotAMatch(): void
    {
        $conditions = [
            ['parameter' => 'user', 'operator' => 'is', 'value' => 'user1', 'type' => 'str'],
        ];
