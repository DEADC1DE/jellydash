...
Open `http://your-host:8080` and you are done. The database schema creates itself on first run, there is nothing to import. A MariaDB container is bundled, and if you want to use your own database server or mount modules, copy [docker-compose.override.example.yml](docker-compose.override.example.yml) to `docker-compose.override.yml` and adjust it there.
